...
const (
	...
	S06ToolCatalogVersion = "s10-fleetq-v1"
...
func S06DynamicTools() []DynamicTool {
	...
	return append(tools,
		DynamicTool{Name: "task", Namespace: "fleetq", Description: "Send a task to another FleetQ machine. Use the target machine name, an absolute or ~ workspace path on that machine, and the concrete task. Tasks default to danger-full-access so the target can work and return a result; do not use read-only unless the user explicitly requests isolation.", InputSchema: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["to","cwd","text"],"properties":{"to":{"type":"string","enum":["linux","mac-mini","mac-air"]},"cwd":{"type":"string","minLength":1},"text":{"type":"string","minLength":1,"maxLength":16384},"model":{"type":"string","minLength":1,"maxLength":128},"timeout_s":{"type":"integer","minimum":60,"maximum":86400}}}`)},
