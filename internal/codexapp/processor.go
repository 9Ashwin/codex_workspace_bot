...
const (
	...
	S06ToolCatalogVersion = "s10-fleetq-v2"
...
func S06DynamicTools() []DynamicTool {
	...
	return append(tools,
		DynamicTool{Name: "task", Namespace: "fleetq", Description: "Send an asynchronous task to another FleetQ machine. The immediate response means accepted by NATS, not completed. The result is returned to the originating Feishu conversation. Set notify_target=true only when the target Bot should also show the result in the current trusted Feishu conversation; the route is derived by the server.", InputSchema: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["to","cwd","text"],"properties":{"to":{"type":"string","enum":["linux","mac-mini","mac-air"]},"cwd":{"type":"string","minLength":1},"text":{"type":"string","minLength":1,"maxLength":16384},"model":{"type":"string","minLength":1,"maxLength":128},"timeout_s":{"type":"integer","minimum":60,"maximum":86400},"notify_target":{"type":"boolean","description":"可选。为 true 时，目标 Bot 还会把结果发到当前可信飞书会话；服务端从当前会话路由生成通知地址，模型不能直接指定 app_id 或 receive_id。"}}}`)},
